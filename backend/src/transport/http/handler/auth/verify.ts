...
export function registerVerifyRoutes(app: Hono, deps: VerifyHandlerDeps) {
  ...
  app.post('/api/auth/verify', zValidator('json', verifySchema), async (c) => {
    ...
    const valid = await traced('auth.verifySignature', () =>
      verifier.verifyWalletSignature(body.address, body.nonce, body.signature),
    );
    if (!valid) {
      return c.json(error('unauthorized', 'Nonce not found, expired, or signature mismatch'), 401);
    }

    const { token, expiresAt } = await traced('auth.issueToken', () =>
      verifier.issueSessionToken(body.address),
    );
    return c.json(
      success({
        session_token: token,
